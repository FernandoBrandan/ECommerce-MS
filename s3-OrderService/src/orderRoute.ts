...
router.post("/:userId", async (req, res) => {
    ...
    try {
        ...
        if (productChecks.includes(false)) return res.status(400).json({ msg: "One or more items are out of stock" })

        // Create new order
        const order = new Order({ userId, items, totalAmount, })
        await order.save()
        // Deduct product stock
        await Promise.all(items.map(async (item) => await axios.put(`${PRODUCT_SERVICE_URI}/api/products/${item.productId}/deduct`, { quantity: item.quantity, })))
